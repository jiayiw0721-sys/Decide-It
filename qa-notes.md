...
The post-collaboration food flow was verified at 390 × 844 without an authenticated session. The homepage contains no login or voting entry point. The food filter now exposes a dedicated regional-cuisine group for 川渝香辣、粤式清鲜、江南鲜甜、西北面香、东北暖胃与云南山野, while maintaining an eight-choice editor ceiling. A direct `sichuan` filter preview produced five concrete regional dishes—重庆小面、麻婆豆腐盖饭、藤椒钵钵鸡、川味回锅肉 and 酸菜鱼—and showed no sign-in requirement.

The final 390 × 844 place and media page review confirmed their primary calls to action now read “Use this short list to decide” and “Use these titles to decide.” Their explanatory copy describes only a single-user random decision, and a complete front-end source scan found no remaining vote or invitation label. The final type check and all 18 unit tests passed.

The food journey was rechecked in English at 390 × 844. All filter headings, filter chips, region names, candidate dishes, and preference controls are now in English. The Sichuan & Chongqing editor preview shows five English dishes and compact “Want it / Either / Not now” preference controls without clipping. Type checking and all 19 unit tests passed, including focused localization coverage for the food filter and regional candidates.

The Vite HMR failure was traced to embedded middleware mode leaving the client port at its default `localhost:5173`. The server now preserves the project server configuration and sets the browser HMR client port to HTTPS port 443 for the managed reverse-proxy preview. After restart, the generated Vite client reported `hmrPort = 443`; opening the external preview produced an empty browser console, with no failed WebSocket message. Type checking and all 19 unit tests passed.

The template editor was checked at 390 × 844. Template-generated food choices now show a visible “这组都不喜欢？换一组” action (and its English equivalent) beneath the candidates. It preserves the selected filters, replaces only the generated options, and shows a gentle message when the current filter has fewer than two alternative candidates. The refresh algorithm is covered by a unit test that verifies the next group shares no candidates with the immediately preceding one; the final suite contains 20 passing tests.

In a live mobile browser check with the narrow 川渝香辣 filter, all five matching specialties were already present in the first group. Tapping “换一组” correctly kept the filter and surfaced the gentle fallback message that no sufficient alternative candidates remain, rather than repeating the same group.

In a second 390 × 844 mobile browser check with the broader 热乎满足 filter, tapping “换一组” retained the 热乎满足 chip and replaced the original group (starting with 重庆小面、麻婆豆腐盖饭) with a non-overlapping set beginning 苏式汤面、本帮红烧肉、腌笃鲜配米饭. The success toast confirmed the refresh, and the editor continued to show eight eligible candidates and the decision controls.
