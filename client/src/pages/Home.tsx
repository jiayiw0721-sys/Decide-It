...
import { SharedVote } from "@/components/SharedVote";
...
type Screen = "today" | "filters" | "editor" | "deciding" | "result" | "records" | "invite" | "vote" | "placeFinder" | "mediaFinder";
...
function getInitialVoteCodeFromUrl(): string | null {
  return new URLSearchParams(window.location.search).get("vote");
}
...
export default function Home() {
  ...
  const [screen, setScreen] = useState<Screen>(() => getInitialVoteCodeFromUrl() ? "vote" : isPlaceSearchPreview() ? "placeFinder" : isMediaPreview() ? "mediaFinder" : getInitialTemplateFromUrl() ? (isEditorPreview() ? "editor" : "filters") : "today");
  ...
  const [shareCode, setShareCode] = useState<string | null>(() => getInitialVoteCodeFromUrl());
  ...
  const goBack = () => {
    ...
    else if (screen === "invite") setScreen("editor");
    else if (screen === "vote") setScreen("today");
    else setScreen("editor");
  };

  const recordsQuery = trpc.decision.list.useQuery(undefined, { enabled: isAuthenticated });
  const sharedDecisionMutation = trpc.sharedDecision.create.useMutation({
    onSuccess: ({ shareCode: code }) => {
      setShareCode(code);
      setScreen("invite");
    },
    onError: () => toast.error("暂时没能创建投票，请稍后再试。"),
  });
  ...
  const beginSharedDecision = () => {
    if (!canDecide) {
      toast.message("请先准备至少两个候选项。", { icon: <UsersRound className="h-4 w-4" /> });
      return;
    }
    if (!isAuthenticated) {
      toast.message("登录后即可发起多人投票。", { icon: <CircleUserRound className="h-4 w-4" /> });
      startLogin();
      return;
    }
    sharedDecisionMutation.mutate({ question: draft.question.trim(), options: validOptions });
  };
  ...
  const usePlaceCandidates = (candidates: ChoiceOption[]) => {
    setDraft((current) => ({ ...current, question: "今天去哪里？", options: candidates, filterLabels: [...(current.filterLabels ?? []), "真实地点"] }));
    setScreen("editor");
  };

  const useMediaCandidates = (candidates: ChoiceOption[]) => {
    setDraft((current) => ({ ...current, question: "今晚看什么？", options: candidates, filterLabels: [...(current.filterLabels ?? []), "TMDb 实时内容"] }));
    setScreen("editor");
  };
  ...
  const screenTitle = screen === "records" ? t("records") : screen === "filters" ? t("choose") : screen === "editor" ? t("edit") : screen === "result" ? t("answer") : screen === "invite" ? t("invite") : screen === "vote" ? t("together") : screen === "placeFinder" ? t("findPlace") : screen === "mediaFinder" ? t("findMedia") : t("today");
    ...
    <div className="min-h-screen bg-[#ebe8e2] text-[#293043] selection:bg-[#e4ddff] selection:text-[#443586]">
      <div className="relative mx-auto min-h-screen max-w-[520px] overflow-hidden bg-[#fbfaf7] shadow-[0_0_50px_rgba(38,45,63,0.12)] md:min-h-[900px] md:my-8 md:rounded-[34px]">
        ...
        <header className="relative z-10 flex items-center justify-between px-6 pb-4 pt-7">
          {screen === "filters" || screen === "editor" || screen === "result" || screen === "deciding" || screen === "invite" || screen === "vote" || screen === "placeFinder" || screen === "mediaFinder" ? (
          ...
          <div className="flex items-center gap-2">
            ...
            {(screen === "today" || screen === "records") && !isAuthenticated && (
              <button onClick={() => startLogin()} className="rounded-full bg-white/80 px-3.5 py-2 text-xs font-semibold text-[#564799] shadow-[0_8px_20px_rgba(42,48,66,0.06)] transition active:scale-95">{t("loginSync")}</button>
            )}
            ...
            {(screen === "filters" || screen === "editor" || screen === "result" || screen === "deciding" || screen === "invite" || screen === "vote" || screen === "placeFinder" || screen === "mediaFinder") && <span className="text-sm font-semibold text-[#50596b]">{screenTitle}</span>}
          </div>
        </header>
        ...
        <main className="relative z-10 px-6 pb-28">
          <AnimatePresence mode="wait">
            ...
            {screen === "editor" && <EditorScreen key="editor" draft={draft} canDecide={canDecide} onChange={setDraft} onOptionChange={updateOption} onOptionRemove={removeOption} onStart={beginDecision} onStartShared={beginSharedDecision} sharing={sharedDecisionMutation.isPending} />}
            ...
            {screen === "invite" && shareCode && <InviteScreen key="invite" shareCode={shareCode} onOpen={() => setScreen("vote")} />}
            {screen === "vote" && shareCode && <SharedVote key="vote" shareCode={shareCode} onBack={() => setScreen("today")} />}
          </AnimatePresence>
        </main>
...
function EditorScreen({ draft, canDecide, onChange, onOptionChange, onOptionRemove, onStart, onStartShared, sharing }: { draft: Draft; canDecide: boolean; onChange: (draft: Draft) => void; onOptionChange: (id: string, patch: Partial<ChoiceOption>) => void; onOptionRemove: (id: string) => void; onStart: () => void; onStartShared: () => void; sharing: boolean }) {
  ...
  return <motion.section initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.25 }}>
    ...
    <button onClick={onStart} className={`mt-9 flex w-full items-center justify-center gap-2 rounded-[19px] py-4 text-[15px] font-bold transition active:scale-[0.98] ${canDecide ? "bg-[#6955b3] text-white shadow-[0_14px_25px_rgba(105,85,179,0.25)]" : "bg-[#e8e6e1] text-[#a1a5ad]"}`}><Sparkles className="h-4 w-4" />{t("randomNow")}</button><button onClick={onStartShared} disabled={sharing} className="mt-3 flex w-full items-center justify-center gap-2 rounded-[19px] border border-[#d9d1f3] bg-white py-3.5 text-sm font-bold text-[#6754ae] transition active:scale-[0.98] disabled:opacity-60">{sharing ? <Loader2 className="h-4 w-4 animate-spin" /> : <UsersRound className="h-4 w-4" />}{t("voteWithFriends")}</button>
  </motion.section>;
}
